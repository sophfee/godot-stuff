...
func _process(delta: float):
	...
	var rightVector: Vector3 = Vector3(
		cos(Player.rotation.y),
		0,
		-sin(Camera.rotation.x)
	);
	var rd: float = rightVector.dot(Player.velocity.limit_length(20) / 20);
	var fd: float = Player.velocity.limit_length(1).length() * .01;
	
	
	rotation = Camera.rotation + Vector3(AccelerationY * .2, (AccelerationX * -.28) - (rd * .02), AccelerationX - (rd * 1));
